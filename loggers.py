...
class SummaryWriterCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls % self._log_freq == 0:
            ...
            _rewards = ['reward', "reward_ee", 'reward_reliability', 'outage_violations',"reward_handover"]
            for _reward_name in _rewards:
                self.tb_formatter.writer.add_scalar(f"rewards/{_reward_name}",
                                                    _info_dict[_reward_name],
                                                    self.n_calls)
