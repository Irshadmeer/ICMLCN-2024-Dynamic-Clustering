...
def render_animation(info_list,max_coordinates):
    ...
    def update(timestep: dict):
        ...
        users.set_offsets(user_locations[:, :2])
        _users_active = timestep['status']
        users.set_alpha(_users_active)
        ...
        for ((user, base_station), connection) in zip(itertools.product(range(num_users), range(num_bs)), connections):
            ...
            linewidth = linewidth * _users_active[user]
    ...
    anim = FuncAnimation(fig, update, frames=info_list, init_func=init,
                         blit=True,interval=100)#interval is delay between the frames
    

    filename = "Train_anim.mp4"
    counter = random.randint(1, 10)

    while os.path.exists(filename):
        filename = f"Train_anim_{counter}.mp4"
        counter += 1
